...
function SignUpScreen({ onSignUp, onGoLogin }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (data.error) { setError(data.error_description || "Sign up failed."); setLoading(false); return; }
      // Upload avatar if provided
      let avatarUrl = null;
      if (avatar && data.access_token) {
        avatarUrl = await sb.uploadPhoto(avatar, `avatars/${data.user.id}_avatar.jpg`, data.access_token);
      }
      if (avatarUrl && data.access_token) {
        await sb.update("profiles", `id=eq.${data.user.id}`, { avatar_url: avatarUrl }, data.access_token);
      }
      saveSession({ token: data.access_token, userId: data.user.id });
      ...
      {step==="photo" && (
        <div style={{ flex:1, padding:"28px 24px", display:"flex", flexDirection:"column", gap:20, alignItems:"center" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10, width:"100%" }}>
            ...
            <button onClick={handleCreate} style={{ background:"none", border:"none", color:GRAY, fontSize:14, cursor:"pointer", fontFamily:"inherit" }}>
              Skip for now
            </button>
          </div>
        </div>
